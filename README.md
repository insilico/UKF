...
2. For each time step `\(k = 1, \ldots, N\)`:
   ...
   2. Propagate each sigma point forward using the ODE model and RK4 integration:
      ...
      ```
      χ_i^{(k|k-1)} = f_{RK4}(χ_i^{(k-1)})
      ```

   3. Compute the predicted state mean and covariance:

      ```
      \hat{\mathbf{x}}_{k|k-1} = ∑_i W_i^m χ_i^{(k|k-1)}
      P_{k|k-1} = ∑_i W_i^c (χ_i^{(k|k-1)} - \hat{\mathbf{x}}_{k|k-1})(χ_i^{(k|k-1)} - \hat{\mathbf{x}}_{k|k-1})^T
      ```

   4. Map sigma points through the observation function:

      ```
      \mathbf{y}_i^{(k|k-1)} = h(χ_i^{(k|k-1)})
      ```

   5. Compute the predicted measurement mean and covariance:

      ```
      \hat{\mathbf{y}}_{k|k-1} = ∑_i W_i^m \mathbf{y}_i^{(k|k-1)}
      S_k = ∑_i W_i^c (\mathbf{y}_i^{(k|k-1)} - \hat{\mathbf{y}}_{k|k-1})(\mathbf{y}_i^{(k|k-1)} - \hat{\mathbf{y}}_{k|k-1})^T + R
      ```

   6. Compute the cross-covariance and Kalman gain:

      ```
      C_k = ∑_i W_i^c (χ_i^{(k|k-1)} - \hat{\mathbf{x}}_{k|k-1})(\mathbf{y}_i^{(k|k-1)} - \hat{\mathbf{y}}_{k|k-1})^T
      K_k = C_k S_k^{-1}
      ```
